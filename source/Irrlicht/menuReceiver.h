...
namespace irr
{
	...
	class IRRLICHT_API menuReceiver : public IEventReceiver
	{
		public: 
			...
			//this is the function that will position the buttons in the world. You can give your own function yourself
			typedef rect<s32> (menuReceiver::*makeButton)(int, int, int);
			...
			void init_custom(IGUIEnvironment* guienv, int width, int height, int amount_buttons, const wchar_t *name[], const wchar_t *sub_name[]); //, makeButton makebutton);
			...
			typedef bool (menuReceiver::*PASS_TURN)(void);
			typedef bool (menuReceiver::*SURRENDER)(void);
			...
			bool menuSwitchCases(const SEvent& event, IGUIEnvironment* guienv, JOIN_GAME join_game, HOST_GAME host_game, JOIN_GAME_SECOND join_game_second, END_GAME end_game, PASS_TURN pass_turn, SURRENDER surrender);
